  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4 sm:space-y-6">
      ...
      <div className="flex items-center gap-3">
        <Link href={`/projects/${id}`} className="p-2 text-gray-500 hover:text-gray-900 dark:text-slate-400 dark:hover:text-white rounded-lg hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors">
          <ArrowLeft size={20} />
        </Link>
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-slate-100">Edit Project</h1>
          <p className="text-xs sm:text-sm text-gray-500 dark:text-slate-400">Update project details and status</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl sm:rounded-2xl border border-gray-200 dark:border-slate-800 shadow-xs p-4 sm:p-6 transition-colors">
        <form action={action} className="space-y-4 sm:space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 sm:gap-4">
            ...
            <div>
              <label htmlFor="deadline" className="block text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1">Deadline</label>
              <input 
                type="date" 
                id="deadline" 
                name="deadline"
            ...
            <div className="sm:col-span-2">
              <label htmlFor="status" className="block text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1">Status</label>
              <select 
                id="status" 
                name="status" 
            ...
            <div className="sm:col-span-2">
              <label htmlFor="notes" className="block text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1">Notes</label>
              <textarea 
                id="notes" 
                name="notes" 
                rows={2}
