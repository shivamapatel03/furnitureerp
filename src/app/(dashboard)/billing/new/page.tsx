    ...
    <div className="max-w-5xl mx-auto space-y-4 sm:space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-500 dark:text-slate-400">Fast invoice generator</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        ...
        <div className="bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-gray-200 dark:border-slate-800 shadow-xs transition-colors">
          <h2 className="text-sm sm:text-base font-bold text-gray-900 dark:text-slate-100 mb-3 border-b border-gray-100 dark:border-slate-800 pb-2">
            Customer Details
          </h2>
        ...
        <div className="bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-gray-200 dark:border-slate-800 shadow-xs transition-colors">
          <div className="flex justify-between items-center mb-3 border-b border-gray-100 dark:border-slate-800 pb-2">
            <h2 className="text-sm sm:text-base font-bold text-gray-900 dark:text-slate-100">Products & Items</h2>
            <button
              type="button"
              onClick={addItem}
              className="text-primary active:scale-95 font-semibold flex items-center gap-1 text-xs sm:text-sm bg-primary/10 hover:bg-primary/20 dark:bg-primary/20 dark:hover:bg-primary/30 px-3 py-1.5 rounded-xl transition-all"
            >
          ...
          {items.length === 0 ? (
            <div className="text-center py-8 px-4 text-gray-500 dark:text-slate-400 bg-gray-50/70 dark:bg-slate-800/40 rounded-xl border border-dashed border-gray-300 dark:border-slate-700">
              ...
              <p className="text-xs text-gray-400 dark:text-slate-500 mt-1 mb-3">Tap 'Add Item' to select products from your inventory.</p>
              <button
                type="button"
                onClick={addItem}
            ...
            <div className="space-y-3">
              {/* Desktop Header */}
              <div className="hidden sm:grid grid-cols-12 gap-3 text-xs font-semibold text-gray-500 dark:text-slate-400 px-3">
              ...
              {items.map((item) => {
                ...
                return (
                  <div 
                    key={item.id} 
                    className="p-3 sm:p-2.5 rounded-xl bg-gray-50/80 dark:bg-slate-800/50 border border-gray-200/70 dark:border-slate-700/60 flex flex-col sm:grid sm:grid-cols-12 gap-2 sm:gap-3 sm:items-center"
                  >
                    {/* Product Selection */}
                    <div className="sm:col-span-5">
                      <label className="block sm:hidden text-[11px] font-semibold text-gray-500 dark:text-slate-400 mb-1">Product</label>
                      <select
                        required
                      ...
                      >
                        ...
                        {products.map(p => (
                          <option key={p.id} value={p.id} className="text-gray-900 dark:text-slate-100 dark:bg-slate-900">{p.name} (Stock: {p.stock} {p.unit})</option>
                        ))}
                      </select>
                    </div>
                    ...
                    <div className="grid grid-cols-3 sm:contents gap-2 items-center pt-1 sm:pt-0">
                      <div className="sm:col-span-2">
                        <label className="block sm:hidden text-[11px] font-semibold text-gray-500 dark:text-slate-400 mb-1">Qty</label>
                        <input
                          type="number"
                          required
                          min="0.1"
                      ...
                      <div className="sm:col-span-2">
                        <label className="block sm:hidden text-[11px] font-semibold text-gray-500 dark:text-slate-400 mb-1">Price (₹)</label>
                        <input
                          type="number"
                          required
                          min="0"
                      ...
                      <div className="sm:col-span-2 sm:text-right flex flex-col justify-end">
                        <span className="block sm:hidden text-[11px] font-semibold text-gray-500 dark:text-slate-400 mb-1">Total</span>
                        <p className="font-extrabold text-gray-900 dark:text-slate-100 text-sm py-2 sm:py-0">₹{item.total.toLocaleString()}</p>
                      </div>

                      <div className="col-span-3 sm:col-span-1 text-right pt-1 sm:pt-0 border-t sm:border-t-0 border-gray-200 dark:border-slate-700">
                        <button
                          type="button"
                          onClick={() => removeItem(item.id)}
                          className="w-full sm:w-auto text-red-500 hover:text-red-700 active:bg-red-100 dark:active:bg-red-950/40 p-1.5 rounded-lg flex items-center justify-center gap-1 text-xs font-semibold"
                          title="Remove item"
                        >
                          <Trash2 size={16} />
