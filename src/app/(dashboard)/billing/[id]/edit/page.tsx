...
import { getBillById, updateBill } from "@/app/actions/billing";
import { getSettings } from "@/app/actions/settings";
import { Plus, Trash2, CheckCircle2, FileText, ArrowLeft, Ruler, Package, Home, Utensils, Building, Hotel, Store, Edit3, Loader2 } from "lucide-react";
...
export default function EditBillPage(props: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    Promise.all([getBillById(id), getProducts(), getSettings()]).then(([bill, prods, settings]) => {
      if (!bill) {
        setNotFound(true);
        setLoading(false);
        return;
      ...
      setCustomerMobile(bill.customer.mobile);

      // Resolve category
      const matchedCategory = PRESET_CATEGORIES.find(c => c.label === bill.category || c.id === bill.category);
      if (matchedCategory) {
        setCategoryType(matchedCategory.id);
      ...
      // Map bill items
      const mappedItems: BillItem[] = bill.items.map(item => ({
        id: item.id || Math.random().toString(),
        productId: item.productId,
        calculationType: (item.calculationType as "UNIT" | "SQFT") || (item.sqft ? "SQFT" : "UNIT"),
      ...
      setTax(bill.tax);
      setPaymentStatus(bill.paymentStatus as any);
      setNotes(bill.notes || "");

      const totalPaid = bill.payments.reduce((acc, p) => acc + p.amount, 0);
      setPaidAmount(totalPaid);
      if (bill.payments.length > 0) {
        setPaymentMethod(bill.payments[0].method as any);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      router.push(`/billing/${id}/print`);
    } else {
      alert(result.error || "Failed to update bill");
    }
  };
  ...
  if (notFound) {
    return (
      <div className="p-12 text-center text-gray-500">
        <h2 className="text-xl font-bold text-gray-900 dark:text-slate-100 mb-2">Invoice Not Found</h2>
        <p className="text-sm text-gray-500 dark:text-slate-400 mb-4">The bill you are trying to edit does not exist.</p>
        <Link href="/billing" className="px-4 py-2 bg-primary text-white rounded-lg text-sm font-semibold">
          Back to Billing
        </Link>
      </div>
    );
    ...
    <div className="max-w-5xl mx-auto space-y-4 sm:space-y-6 pb-12">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <Link href={`/billing/${id}/print`} className="p-2 text-gray-500 hover:text-gray-900 dark:text-slate-400 dark:hover:text-white rounded-lg hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors">
            <ArrowLeft size={20} />
          </Link>
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-slate-100">Edit Invoice</h1>
              <span className="px-2.5 py-0.5 bg-red-100 dark:bg-red-950/60 text-red-700 dark:text-red-300 text-xs font-extrabold rounded-md">
                {billNumber}
              </span>
            </div>
            <p className="text-xs sm:text-sm text-gray-500 dark:text-slate-400">Modify items, sqft pricing, category, and payments</p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-gray-200 dark:border-slate-800 shadow-xs transition-colors">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3 border-b border-gray-100 dark:border-slate-800 pb-2">
            <div>
              ...
              <p className="text-[11px] text-gray-400 dark:text-slate-500">
                Default Sqft Base Rate: <span className="font-bold text-primary">₹{defaultSqftRate}/sqft</span>
              </p>
            </div>
          ...
          {items.length === 0 ? (
            <div className="text-center py-8 px-4 text-gray-500 dark:text-slate-400 bg-gray-50/70 dark:bg-slate-800/40 rounded-xl border border-dashed border-gray-300 dark:border-slate-700">
              <p className="text-sm font-medium text-gray-700 dark:text-slate-300">No items in this invoice</p>
          ...
          <div className="bg-gray-50 dark:bg-slate-900/90 p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-gray-200 dark:border-slate-800 shadow-xs flex flex-col justify-between space-y-4 transition-colors">
            ...
            <div className="flex items-center gap-3 pt-3">
              <Link
                ...
                className="w-1/3 text-center py-3.5 px-4 bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 text-gray-700 dark:text-slate-300 font-bold rounded-xl active:scale-98 transition-all text-sm"
              >
                Cancel
              </Link>
              <button
                ...
                className="w-2/3 bg-primary hover:bg-primary-dark active:scale-98 text-white font-bold py-3.5 px-4 rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-md text-sm sm:text-base"
              >
                {submitting ? (
                  <>
