  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4 sm:space-y-6">
      ...
      <div className="flex items-center gap-3">
        <Link href="/material-usage" className="p-2 text-gray-500 hover:text-gray-900 rounded-lg hover:bg-gray-100 transition-colors">
          <ArrowLeft size={20} />
        </Link>
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Log Material Usage</h1>
          <p className="text-xs sm:text-sm text-gray-500">Record inventory consumed at a project site</p>
        </div>
      </div>
      ...
      <div className="bg-white p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-gray-200 shadow-xs">
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-700 mb-1">Product / Material *</label>
            <select
              required
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 sm:gap-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1">Area / Room Location</label>
              <input
                type="text"
                value={formData.area}
          ...
          <div className="pt-4 flex flex-col-reverse sm:flex-row justify-end gap-2.5 sm:gap-3 border-t border-gray-100">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full sm:w-auto px-6 py-2.5 text-sm font-bold text-white bg-primary hover:bg-primary-dark active:scale-98 rounded-xl transition-all shadow-sm disabled:opacity-50"
            >
