...
export default function AttendanceManager({
  ...
  const inputCls = "w-full px-3.5 py-2 border border-gray-300 rounded-xl text-sm outline-none focus:ring-2 focus:ring-primary";
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl border border-gray-200 shadow-xs overflow-hidden">
          ...
          <div className="block sm:hidden divide-y divide-gray-100">
                ...
                <div key={emp.id} className="p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-gray-400">{emp.position || "General Staff"}</p>
                    </div>
                    <span className="text-xs font-extrabold text-gray-700 bg-gray-100 px-2 py-1 rounded-lg">
                      ₹{emp.dailySalary}/day
                    </span>
                  </div>
            ...
            {employees.length === 0 && (
              <div className="p-8 text-center text-gray-500">
                ...
                <p className="text-base font-bold text-gray-900">No staff registered</p>
                <p className="text-xs text-gray-400 mt-1 mb-4">Go to the 'Staff List' tab to add your team members.</p>
                <button
          ...
          <div className="hidden sm:block overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 text-gray-500 text-xs uppercase font-semibold tracking-wider border-b border-gray-100">
                ...
                {employees.map(emp => {
                  ...
                  return (
                    <tr key={emp.id} className="hover:bg-gray-50/70 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right text-gray-800 font-extrabold">₹{emp.dailySalary.toLocaleString()}/day</td>
      ...
      {tab === "staff" && (
        <div className="bg-white rounded-xl sm:rounded-2xl border border-gray-200 shadow-xs overflow-hidden">
          <div className="p-3.5 sm:p-4 border-b border-gray-100 bg-gray-50/60 flex justify-between items-center">
            <div>
              <h2 className="font-bold text-gray-900 text-sm sm:text-base">Staff Members ({employees.length})</h2>
              <p className="text-xs text-gray-500">Manage carpenters, helpers & workers</p>
            </div>
            <button 
              onClick={() => { setShowAddForm(true); setEditingId(null); }}
              className="flex items-center gap-1.5 bg-primary hover:bg-primary-dark active:scale-95 text-white px-3.5 py-2 rounded-xl text-xs sm:text-sm font-semibold transition-all shadow-xs"
            >
          ...
          {showAddForm && (
            <div className="p-4 border-b border-gray-200 bg-blue-50/50 space-y-3">
              <p className="text-xs font-bold text-blue-900 uppercase tracking-wider">New Staff Member</p>
              ...
              <div className="flex justify-end gap-2 pt-1">
                <button 
                  ...
                  className="px-3.5 py-2 text-xs font-semibold text-gray-600 bg-white border border-gray-300 rounded-xl hover:bg-gray-50"
                >
                ...
                <button 
                  ...
                  className="px-4 py-2 text-xs font-bold text-white bg-primary rounded-xl hover:bg-primary-dark disabled:opacity-50 shadow-xs"
                >
              ...
              <div key={emp.id} className="p-4 space-y-2">
                {editingId === emp.id ? (
                  <div className="space-y-2.5">
                    ...
                    <div className="flex justify-end gap-2 pt-1">
                      <button onClick={() => setEditingId(null)} className="px-3 py-1.5 text-xs text-gray-600 bg-gray-100 rounded-lg">Cancel</button>
                  ...
                  <>
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-xs text-gray-600 mt-0.5">{emp.mobile}</p>
                      </div>
                      <div className="text-right">
                        <span className="font-extrabold text-gray-900 text-sm">₹{emp.dailySalary}</span>
                    ...
                    <div className="pt-2 flex justify-between items-center border-t border-gray-50">
                      <span className="text-xs text-gray-400">Monthly: ₹{(emp.dailySalary * 30).toLocaleString()}</span>
                      <div className="flex items-center gap-1">
                        <button 
                          onClick={() => startEdit(emp)} 
                          className="p-1.5 text-gray-500 hover:text-primary rounded-lg"
                        >
                          <Edit2 size={16} />
                        </button>
                        <button 
          ...
          <div className="hidden sm:block overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                  ...
                  <tr key={emp.id} className="hover:bg-gray-50/70 transition-colors">
                    {editingId === emp.id ? (
                      <>
                        ...
                        <td className="px-4 py-2 text-right text-gray-500 text-xs">₹{(parseFloat(formData.dailySalary || "0") * 30).toLocaleString()}</td>
                        <td className="px-4 py-2 text-right">
                          <button onClick={() => handleUpdate(emp.id)} className="p-1.5 text-green-600 hover:bg-green-50 rounded"><Check size={16} /></button>
                          <button onClick={() => setEditingId(null)} className="p-1.5 text-gray-400 hover:bg-gray-100 rounded ml-1"><X size={16} /></button>
                        </td>
                      </>
                    ) : (
                      <>
                        <td className="px-6 py-4 font-bold text-gray-900">{emp.name}</td>
                        <td className="px-6 py-4 text-gray-500">{emp.mobile}</td>
                        <td className="px-6 py-4 text-gray-500">{emp.position || "—"}</td>
                        <td className="px-6 py-4 text-right font-extrabold text-gray-900">₹{emp.dailySalary.toLocaleString()}</td>
                        <td className="px-6 py-4 text-right text-gray-600 font-semibold">₹{(emp.dailySalary * 30).toLocaleString()}</td>
                        <td className="px-6 py-4 text-right">
                          <button onClick={() => startEdit(emp)} className="p-2 text-gray-400 hover:text-primary hover:bg-primary/10 rounded-lg transition-colors"><Edit2 size={16} /></button>
                          <button onClick={() => handleDelete(emp.id, emp.name)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors ml-1"><Trash2 size={16} /></button>
                        </td>
                      </>
                    )}
                  </tr>
                ...
                {employees.length === 0 && (
                  <tr><td colSpan={6} className="px-6 py-12 text-center text-gray-400">No staff added yet. Click "Add Staff" to get started.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl border border-gray-200 shadow-xs overflow-hidden">
          ...
          <div className="block sm:hidden divide-y divide-gray-100">
            {initialSalary.map(row => (
              <div key={row.employee.id} className="p-4 space-y-2.5">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="font-bold text-gray-900 text-sm">{row.employee.name}</h3>
                    <p className="text-xs text-gray-400">{row.employee.position || "Staff"}</p>
                  </div>
                  <div className="text-right">
                ...
                <div className="grid grid-cols-3 gap-2 bg-gray-50 p-2.5 rounded-xl text-center text-xs">
                  <div>
                    <span className="text-gray-400 text-[10px] block">Present</span>
                    <span className="font-bold text-green-600">{row.presentDays} d</span>
                  </div>
                  <div>
                    <span className="text-gray-400 text-[10px] block">Half Day</span>
                    <span className="font-bold text-amber-600">{row.halfDays} d</span>
                  </div>
                  <div>
                    <span className="text-gray-400 text-[10px] block">Absent</span>
                    <span className="font-bold text-red-600">{row.absentDays} d</span>
                  </div>
                </div>
            ...
            {initialSalary.length === 0 && (
              <div className="p-8 text-center text-gray-400 text-sm">No employee data found.</div>
            )}
          </div>
          ...
          <div className="hidden sm:block overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {initialSalary.map(row => (
                  <tr key={row.employee.id} className="hover:bg-gray-50/70 transition-colors">
                    <td className="px-6 py-4 font-bold text-gray-900">{row.employee.name}</td>
                    <td className="px-6 py-4 text-right text-gray-600">₹{row.employee.dailySalary.toLocaleString()}</td>
                ...
                {initialSalary.length === 0 && (
                  <tr><td colSpan={7} className="px-6 py-12 text-center text-gray-400">No staff data available.</td></tr>
                )}
              </tbody>
            </table>
          </div>
